- [껍데기와 서비스, DAO에 대한 궁금사항](#껍데기와-서비스-dao에-대한-궁금사항)
  - [왜 껍데기와 서비스를 나누는가?](#왜-껍데기와-서비스를-나누는가)
  - [왜 DAO를 쓰는가?](#왜-dao를-쓰는가)
...
- [CSS](#css)
- [버튼 만들기](#버튼-만들기)
    - [텍스트의 길이에 따라 버튼의 크기가 비례하게 변한다.](#텍스트의-길이에-따라-버튼의-크기가-비례하게-변한다)
    - [반응형 버튼 만들기](#반응형-버튼-만들기)
    - [btn의 크기는 같지만 폰트의 수와 색상이 다를경우](#btn의-크기는-같지만-폰트의-수와-색상이-다를경우)
    - [btn의 비활성화상태](#btn의-비활성화상태)


# 껍데기와 서비스, DAO에 대한 궁금사항

## 왜 껍데기와 서비스를 나누는가?

- 껍데기 수정과 서비스 수정을 분리하여 수정 비용(시간,금액 등)을 최소화시킬 수 있다.

## 왜 DAO를 쓰는가?

- 업무로직을 담당하는 사람이 데이터를 몰라도 업무 수행에 지장이 없다.
- 데이터 소스가 바뀌더라도 서비스 쪽은 신경쓰지않고 업무를 지속할수 있다.
- +함수 집중화
- DAO의 역할
    - 자바에서 CRUD 말고도 **정렬, 집계 등등의 데이터 관련 처리를 가능하게 해준다.**
