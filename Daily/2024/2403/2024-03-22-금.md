...
- [스프링 컨테이너와 스프링 빈](#스프링-컨테이너와-스프링-빈)
  ...
  - [컨테이너에 등록된 모든 빈 조회하기](#컨테이너에-등록된-모든-빈-조회하기)
  - [스프링 빈 조회하기](#스프링-빈-조회하기)
    - [같은 타입이 두개 이상 있을 경우](#같은-타입이-두개-이상-있을-경우)
...
- 자기자신 안에 있는 함수를 호출하는 것
- 제한을 두지 않으면 무한 루프에 빠질수 있기 때문에 제한을 두어야 한다.
...
- ‘#피라미드’ 그리는 함수로 재귀를 사용하면 반복문을 중첩으로 사용할 필요가 없다.
- draw함수 내에 draw()를 호출한다. → 재귀
- 조건문을 이용하여 제약사항을 두어 무한 루프를 방지한다.
