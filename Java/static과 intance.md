- [static이란?](#static이란)
- [static을 사용하는 이유](#static을-사용하는-이유)
- [static 특징](#static-특징)
- [static 변수](#static-변수)
  - [예시 - 클래스 객체 생성 개수 세기](#예시---클래스-객체-생성-개수-세기)
    - [변수와 생명주기](#변수와-생명주기)
    - [static 변수의 접근](#static-변수의-접근)
    - [static 변수 정리](#static-변수-정리)
...
- [용어 정리](#용어-정리)
    - [멤버 변수의 종류](#멤버-변수의-종류)
    - [멤버 메서드의 종류](#멤버-메서드의-종류)
- [static 사용시 주의사항](#static-사용시-주의사항)


이번에는 `public static void main(String[] args)`을 많이 쓰는데, 여기서 static이 무엇인지 알아보고자 한다. 왜 static을 사용할까? 

# static이란?

- 메소드나 변수를 전역화시켜 모든 클래스가 공유할 수 있도록 만드는 예약어

# static을 사용하는 이유

- 만약 클래스가 생성된 개수(count)를 세고자 한다면 어떻게 해야 할까? 먼저 count 변수를 클래스끼리 공유를 시켜 A클래스가 생성되면 +1, B클래스가 생성되면 +2… 식으로 추가되어야한다. 이때 사용하는 것이 static이다. 메소드도 마찬가지이다.
...
- 모든 클래스가 공유할 수 있는 변수
- static변수, 정적 변수, 클래스 변수 라고도 한다.
- static 변수는 힙 영역이 아닌 메소드 영역에서 관리한다. 쉽게 생각해서 책상에 적혀있는 정보와 내 공책에 있는 정보를 떠올리면 되는데, 책상에 적혀있는 정보는 나뿐만 아니라 이 책상을 이용하는 모든 사람들이 볼 수 있다. 반면에, 내 공책에 있는 건 나만 볼 수 있고, 다른 사람들은 동의를 구해야 볼 수 있다.
    - 책상 = 메소드 영역, 내 공책 = 힙 영역
    - 책상에 있는 정보 : static 변수, 내 공책에 있는 정보 = instance 변수
    - 책상이 1개면 static 변수도 1개만 존재한다. (1책상 1정보), 반면에 공책은 공책 수만큼 변수가 존재한다.

## 예시 - 클래스 객체 생성 개수 세기

먼저 static을 이용하지 않고 객체 생성 개수(count)를 세보기로 하자. 
...
- 모든 인스턴스가 공유하는 변수로 static을 이용하면 우리가 사용했던 Counter 처럼 따로 클래스를 생성하지 않고 Data 클래스 만으로도 공유하는 변수를 만들 수 있다.
...
- staticCall()을 5번 이용한다고 가정해보자.
    ...
    - 다음처럼 DecoData를 직접 다 작성해주어야하는 번거로움이 존재하는데 이를 요약시켜주는 것이 `static import`이다.
    
    ```java
    import static static2.DecoData.*; // static import
...
- **그럼 왜 static 메서드를 사용할까?**
    - main메서드는 프로그램을 실행하는 중요한 메서드인데, 이는 **객체 생성없이 프로그램을 실행하도록 하기 위해서 static을 붙여준 것**이다.
...
1. **static 변수의 긴 생명주기**
    - 사용하던 사용하지 않던 프로그램이 실행하는 내내 변수가 메모리에 존재하기 때문에, static 변수가 많아질수록 메모리 낭비가 일어날 수 있다.
    - Garbage Collector로 회수되지 않는다.
2. **오버라이딩 불가**
    - 정적으로 선언된 변수/메서드는 오버라이딩이 불가하여 확장성이 제한된다.
3. **데이터 추적의 어려움**
    - static은 전역에서 관리되기 때문에 해당 데이터의 흐름을 파악하기 어렵다.
4. **Thread 불안정성**
    - 전역에서 사용되기 때문에 모든 Thread에서 static 필드를 공유한다. 이때, 특정 Thread에서 변수 값을 변경하면 다른 모든 Thread에 영향을 줄 수 있다.
    - 만약 변경을 불가능하게 하려면 `final`을 이용해보자.

출처

- https://tcpschool.com/java/java_modifier_ectModifier
- https://tcpschool.com/java/java_member_field
...
- [https://www.incodom.kr/정적변수(static_variable)#h_ef3271c2db875289089dc08f1acc5aff](https://www.incodom.kr/%EC%A0%95%EC%A0%81%EB%B3%80%EC%88%98%28static_variable%29#h_ef3271c2db875289089dc08f1acc5aff)